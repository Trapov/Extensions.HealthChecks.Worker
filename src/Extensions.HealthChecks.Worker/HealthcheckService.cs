namespace Extensions.HealthChecks.Worker
{
    ...
    public class HealthCheckWorker : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            {
                /*
                    Порты должны быть открыты для пользователя, под именем которого запускается сервис
                    Под админом в консоли для всех префиксов:
                    netsh http add urlacl url={prefix} user={domain}\{username}
                */
